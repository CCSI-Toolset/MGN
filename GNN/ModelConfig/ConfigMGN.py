...
class ModelConfig:
    def __init__(self, config_file):
        ...
        self.window_length = 5 if 'window_length' not in c_data else c_data['window_length']
        self.apply_onehot = False if 'apply_onehot' not in c_data else c_data['apply_onehot']
        self.boundary_node_types = [1] if 'boundary_node_types' not in c_data else c_data['boundary_node_types']
        self.source_node_types = [2] if 'source_node_types' not in c_data else c_data['source_node_types']
        self.noise = None if 'noise' not in c_data else c_data['noise']
        self.noise_gamma = 0.1 if 'noise_gamma' not in c_data else c_data['noise_gamma']
        self.normalize = True if 'normalize' not in c_data else c_data['normalize']
        ...
        self.mgn_dim = 128 if 'mgn_dim' not in m_data else m_data['mgn_dim']
        self.mp_iterations = 15 if 'mp_iterations' not in m_data else m_data['mp_iterations']
        ...
        self.epochs = 100 if 'epochs' not in t_data else t_data['epochs']
        self.batch_size = 8 if 'batch_size' not in t_data else t_data['batch_size']
        self.scheduler = 'ExpLR' if 'scheduler' not in t_data else t_data['scheduler']
        self.use_parallel = False if 'use_parallel' not in t_data else t_data['use_parallel']
        ...
        # tensorboard
        self.use_tensorboard = False if 'use_tensorboard' not in t_data else t_data['use_tensorboard']
        self.tensorboard_dir = join(
            'tensorboard', self.name) if 'tensorboard_dir' not in t_data else t_data['tensorboard_dir']
        self.tb_rate = 10 if 'tb_rate' not in t_data else t_data['tb_rate']
        self.log_rate = 100 if 'log_rate' not in t_data else t_data['log_rate']
    ...
    #######
        ...
        self.do_rollout_test = False if 'do_rollout_test' not in c_test else c_test['do_rollout_test']
        self.rollout_start_idx = 100 if 'rollout_start_idx' not in c_test else c_test['rollout_start_idx']
