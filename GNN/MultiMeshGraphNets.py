...
class MultiMeshGraphNets(GNN):
    def __init__(
        ...
        **kwargs,
        ...
        in_dim_edges: list of input edge feature dimensions
        ...
        super(MultiMeshGraphNets, self).__init__(
            in_dim_node,
            in_dim_edges,
            out_dim,
            num_edge_types,
        ...
        self._edge_normalizers = ModuleList(
            [
                Normalizer(
                    size=in_dim_edge,
                    ignore_mask=edge_normalizer_mask,
                    name="edge_normalizer_%d" % i,
                )
                for i, in_dim_edge in enumerate(in_dim_edges)
            ]
        )
    ...
    def _forward(self, graph, force_accum_off=False):
        ...
        if type(graph) is tuple:
            x, edge_indices, edge_attrs = graph
        else:
            x = graph.x
        ...
        edge_attrs = [
            edge_encoder(_edge_normalizer(edge_attr, accumulate=accumulate))
            for edge_encoder, _edge_normalizer, edge_attr in zip(
                self.edge_encoders, self._edge_normalizers, edge_attrs
            )
        ]
            ...
            if self.normalize_output:  # f_1 = normalized dy/dt
                f_2 = self._forward(
                    (
                        graph.x
                        + 0.5
                        * pad(self._output_normalizer.inverse(f_1), (0, other_dim)),
                        edge_indices,
                        edge_attrs,
                    ),
    ...
    def heun_forward(self, graph):
        ...
        if not self.full_grad:
            ...
            if self.normalize_output:  # f_1 = normalized dy/dt
                ...
                f_2 = self._forward(
                    (graph.x + pad(f_1, (0, other_dim)), edge_indices, edge_attrs),
                    force_accum_off=True,
                )
    ...
    def rollout(
        ...
        # input_graph = Data(edge_indices=graph.edge_indices, edge_attrs=graph.edge_attrs).to(device)
        input_graph = graph.clone().to(device)
